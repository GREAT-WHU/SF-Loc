...
class DBAFusionFrontend:
    ...
    def __update(self):
        ...
        if self.video.imu_enabled and (self.video.tstamp[self.t1-1] - self.video.vi_init_time > 5.0):
            self.video.reinit = False
            self.video.vi_init_time = 1e9

        # new frame come, append IMU (<5 ms)
        ...
        while self.all_imu[self.cur_imu_ii][0] < cur_t:
            ...
            #         self.video.sgraph.append_imu_temp(float(self.all_stamp[self.cur_stamp_ii][0]),\
            #                         self.all_imu[self.cur_imu_ii][4:7],\
            #                         self.all_imu[self.cur_imu_ii][1:4]/180*math.pi,True)
            #         if float(self.all_stamp[self.cur_stamp_ii][0]) > self.video.sgraph.timestamps[-1] and\
            #               math.fabs(cur_t - float(self.all_stamp[self.cur_stamp_ii][0]))>1e-3:
            #             pose_temp = self.video.sgraph.pose_temp
            ...
            #         self.cur_stamp_ii += 1
            #     self.video.sgraph.append_imu_temp(self.all_imu[self.cur_imu_ii][0],\
            #                         self.all_imu[self.cur_imu_ii][4:7],\
        ...
        # append GNSS
        if len(self.all_gnss) > 0: gnss_found = bisect.bisect(self.all_gnss[:,0],cur_t - 0.05)
        else: gnss_found = -1        
        if gnss_found > 0 and self.all_gnss[gnss_found,0] - cur_t < 0.05 :
            #   and self.all_gnss[gnss_found,16] == 1.0 and self.all_gnss[gnss_found,13] > 10:
        ...
        self.video.disps[self.t1] = self.video.disps[self.t1-1].mean() * 0.75
